...
- Vetrra integrations are always enabled; you just provide URL + API key.
...
- Indexer Name: use the exact indexer name as shown inside Radarr/Sonarr.
  - Where to find it: Radarr/Sonarr ƒ+' Settings ƒ+' Indexers (copy the displayed name)
- Indexer Base URL: your indexerƒ?Ts base URL (example: https://nzbfinder.ws)
...
- Indexer UID: only required by some Newznab indexers (NZBFinder is a common example).
  ...
  - Where to find it (NZBFinder verified):
    ...
    2. Right-click the ƒ?oDownload NZBƒ?? link ƒ+' Copy Link Address.
  ...
  - Where to find it (other indexers): check the indexerƒ?Ts API / API Info page for example URLs.
  - If you canƒ?Tt find a `uid` anywhere, leave it blank and run a preflight test; some indexers donƒ?Tt require it.
